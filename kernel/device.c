        ...
        )
{
    ...
    if (ret < 0) {
        ECAT_ERR(master, "Failed to init debug device!\n");
        goto out_return;
    }
#endif
    ...
    for (i = 0; i < EC_TX_RING_SIZE; i++) {
        if (!(device->tx_skb[i] = dev_alloc_skb(ETH_FRAME_LEN))) {
            ECAT_ERR(master, "Error allocating device socket buffer!\n");
            ret = -ENOMEM;
            goto out_tx_ring;
        }
        ...
        )
{
    ...
    if (!device->dev) {
        ECAT_ERR(device->ecat_node, "No net_device to open!\n");
        return -ENODEV;
    }

    if (device->open) {
        ECAT_WARN(device->ect_node, "Device already opened!\n");
        return 0;
    }
        ...
        )
{
    ...
    if (!device->dev) {
        ECAT_ERR(device->master, "No device to close!\n");
        return -ENODEV;
    }

    if (!device->open) {
        ECAT_WARN(device->master, "Device already closed!\n");
        return 0;
    }
        ...
        )
{
    ...
    if (unlikely(device->ecat_node->debug_level > 1)) {
        ECAT_DBG(device->master, 2, "Sending frame:\n");
        ...
        )
{
    ...
    ECAT_DBG(device->ecat_node, 1, "Debug ring %u:\n", ring_index);
    ...
    for (i = 0; i < device->debug_frame_count; i++) {
        ...
        ECAT_DBG(device->ecat_node, 1, "Frame %u, dt=%u.%06u s, %s:\n",
...
{
    ec_node_t *ecat_node = device->ecat_node;
    char str[20];

    ec_mac_print(device->dev->dev_addr, str);
    ECAT_INFO(master, "Releasing main device %s.\n", str);
...
int ecdev_open(ec_device_t *device /**< EtherCAT device */)
{
    ...
    if (ret) {
        ECAT_ERR(device->master, "Failed to open device!\n");
        return ret;
    }

    ret = ec_master_enter_idle_phase(device->master);
    if (ret) {
        ECAT_ERR(device->master, "Failed to enter IDLE phase!\n");
        return ret;
    }
...
void ecdev_close(ec_device_t *device /**< EtherCAT device */)
{
    ...
    if (ec_device_close(device))
        ECAT_WARN(device->master, "Failed to close device!\n");
}
        ...
        )
{
    ...
    if (unlikely(!data)) {
        ECAT_WARN(device->master, "%s() called with NULL data.\n",
                __func__);
        return;
    }
    ...
    if (unlikely(device->ecat_node->debug_level > 1)) {
        ECAT_DBG(device->ecat_node, 2, "Received frame:\n");
...
{
    if (unlikely(!device)) {
        ECAT_WARN(device->master, "ecdev_set_link(): No device!\n");
        return;
    }

    if (likely(state != device->link_state)) {
        device->link_state = state;
        ECAT_INFO(device->master,
...
{
    if (unlikely(!device)) {
        ECAT_WARN(device->master, "ecdev_get_link(): No device!\n");
        return 0;
    }
