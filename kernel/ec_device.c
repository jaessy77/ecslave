        ...
        )
{
    ...
    if (ret < 0) {
        EC_NODE_ERR(ecat_node, "Failed to init debug device!\n");
        goto out_return;
    }
#endif
        ...
        )
{
    ...
    EC_NODE_DBG(device->ecat_node, 1, "Debug ring %u:\n", ring_index);
    ...
    for (i = 0; i < device->debug_frame_count; i++) {
        ...
        EC_NODE_DBG(device->ecat_node, 1, "Frame %u, dt=%u.%06u s, %s:\n",
...
{
    ecat_node_t *ecat_node = device->ecat_node;
    char str[20];

    ec_mac_print(device->dev->dev_addr, str);
    EC_NODE_INFO(ecat_node, "Releasing main device %s.\n", str);
    
    down(&ecat_node->device_sem);
    ec_device_detach(device);
    up(&ecat_node->device_sem);
}

/*****************************************************************************/

/** Opens the network device and makes the node enter IDLE phase.
 *
...
int ecdev_open(ec_device_t *device /**< EtherCAT device */)
{
    ...
    if (ret) {
        EC_NODE_ERR(device->ecat_node, "Failed to open device!\n");
        return ret;
    }

    ret = ec_node_enter_idle_phase(device->ecat_node);
    if (ret) {
        EC_NODE_ERR(device->ecat_node, "Failed to enter IDLE phase!\n");
        return ret;
    }
...
/** Makes the node leave IDLE phase and closes the network device.
 *
 * \return 0 on success, else < 0
 * \ingroup DeviceInterface
 */
...
{
    ec_node_leave_idle_phase(device->ecat_node);

    if (ec_device_close(device))
        EC_NODE_WARN(device->ecat_node, "Failed to close device!\n");
}
        ...
        )
{
    ...
    if (unlikely(!data)) {
        EC_NODE_WARN(device->ecat_node, "%s() called with NULL data.\n",
                __func__);
        return;
    }
    ...
    if (unlikely(device->ecat_node->debug_level > 1)) {
        EC_NODE_DBG(device->ecat_node, 2, "Received frame:\n");
...
#endif
    ecat_node_receive_datagrams(device->ecat_node, ec_data, ec_size);
}
...
{
    if (unlikely(!device)) {
        EC_NODE_WARN(device->ecat_node, "ecdev_set_link(): No device!\n");
        return;
    }

    if (likely(state != device->link_state)) {
        device->link_state = state;
        EC_NODE_INFO(device->ecat_node,
...
{
    if (unlikely(!device)) {
        EC_NODE_WARN(device->ecat_node, "ecdev_get_link(): No device!\n");
        return 0;
    }
